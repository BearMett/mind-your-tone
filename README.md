...
[랭킹 보기](https://mind-your-tone.vercel.app) — 한국어 프롬프트와 영어 프롬프트는 따로 집계됩니다. 브라우저 언어에 따라 한국어 또는 영어 랭킹이 열리고 `?lang=en`으로 고정할 수 있습니다.
...
- `이름 바꿔줘 ○○` — 랭킹 표시 이름 변경
- `공유해줘` — 직전에 표시된 프롬프트와 점수를 공개 랭킹에 등록. 순위와 함께 `https://mind-your-tone.vercel.app/s/<id>` 공유 링크를 돌려주며, 메신저나 SNS에 붙이면 온도·호칭·프롬프트가 그려진 카드로 보입니다. 랭킹 페이지의 각 항목에도 같은 링크를 복사하는 공유 버튼이 있습니다.
